...
const WALLET_CONFIG = {
    // // Dynamically determine the login URL based on the current environment
    // loginUrl: (function() {
    //     const hostname = window.location.hostname;
    //     // If running on localhost, 127.0.0.1, or a local LAN IP, use the local dev server
    //     if (hostname === 'localhost' || hostname === '127.0.0.1' || hostname.startsWith('192.168.')) {
    //         return 'http://192.168.2.2:5000/wallet_login.html';
    //     }
    //     // Otherwise, default to production
    //     return 'https://hk.rbas.top/wallet_login.html';
    // })(),
...
function checkWalletLoginStatus() {
    try {
        const walletid = localStorage.getItem('gangda_walletid');
        const walletName = localStorage.getItem('gangda_wallet_name');
        const walletPhone = localStorage.getItem('gangda_wallet_phone');
        const loginTime = localStorage.getItem('gangda_wallet_login_time');
...
function clearWalletLogin() {
    localStorage.removeItem('gangda_walletid');
    localStorage.removeItem('gangda_wallet_name');
    localStorage.removeItem('gangda_wallet_phone');
    localStorage.removeItem('gangda_wallet_login_time');
    // 同时清除旧格式的登录数据（兼容）
    localStorage.removeItem('gangda_wallet');
    localStorage.removeItem('gangda_name');
    localStorage.removeItem('gangda_account');
    localStorage.removeItem('gangda_phone');
}

function saveWalletLogin(walletid, name, phone) {
    localStorage.setItem('gangda_walletid', walletid);
    localStorage.setItem('gangda_wallet_name', name || '');
    localStorage.setItem('gangda_wallet_phone', phone || '');
    localStorage.setItem('gangda_wallet_login_time', Date.now().toString());
    // 同时保存旧格式（兼容现有代码）
    localStorage.setItem('gangda_wallet', walletid);
    localStorage.setItem('gangda_name', name || '');
    localStorage.setItem('gangda_account', name || '');
    localStorage.setItem('gangda_phone', phone || '');
}

// ========== 🔥 使用模态框打开钱包道登录（移动端全屏适配版） ==========
function openWalletLogin() {
    return new Promise((resolve, reject) => {
        ...
        modal.style.cssText = `
            ...
            ${isMobile ? 'height: auto; min-height: 96vh; max-height: 98vh;' : ''}
        `;
        ...
        iframeWrapper.style.cssText = `
            ...
            position: relative;
            ${!isMobile ? 'height: auto; min-height: 400px;' : ''}
            ${isMobile ? 'height: auto; min-height: 82vh;' : ''}
        `;
        ...
        if (isMobile) {
            const handleResize = () => {
                ...
                // 让模态框高度接近全屏，但留出安全区域
                const modalHeight = Math.min(vh * 0.98, vh - 10);
                modal.style.height = 'auto';
                modal.style.maxHeight = modalHeight + 'px';
                modal.style.minHeight = '92vh';
                
                // 确保 iframe 也有足够高度
                iframe.style.minHeight = '78vh';
                iframeWrapper.style.minHeight = '78vh';
